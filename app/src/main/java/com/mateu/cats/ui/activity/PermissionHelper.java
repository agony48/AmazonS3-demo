...
import static com.mateu.cats.utils.CheckPermissionUtils.requestPermissions;
...
public class PermissionHelper {
    ...
    private final static int ACCESS_NETWORK_STATE_CODE = 103;
    private final static int INTERNET_CODE = 104;
    private final static int READ_EXTERNAL_STORAGE_CODE = 105;
    private final static int ACCESS_COARSE_LOCATION_CODE = 106;
    private final static int ACCESS_FINE_LOCATION_CODE = 107;
    private final static int ACCESS_WIFI_STATE_CODE = 108;
    ...
    private PermissionModel[] mPermissionModels = new PermissionModel[]{
            new PermissionModel("位置", Manifest.permission.ACCESS_COARSE_LOCATION, "我们需要访问您的位置", ACCESS_COARSE_LOCATION_CODE),
            new PermissionModel("位置搜索", Manifest.permission.ACCESS_FINE_LOCATION, "我们需要搜索您的位置", ACCESS_FINE_LOCATION_CODE),
            new PermissionModel("电话", Manifest.permission.READ_PHONE_STATE, "我们需要读取手机信息的权限来标识您的身份", READ_PHONE_STATE_CODE),
            new PermissionModel("网络状态", Manifest.permission.ACCESS_NETWORK_STATE, "我们需要访问手机的网络状态", ACCESS_NETWORK_STATE_CODE),
            new PermissionModel("网络", Manifest.permission.INTERNET, "我们需要开启手机网络", INTERNET_CODE),
            new PermissionModel("网络状态", Manifest.permission.ACCESS_WIFI_STATE, "我们需要访问手机的网络状态", ACCESS_WIFI_STATE_CODE),
            new PermissionModel("读取存储空间", Manifest.permission.READ_EXTERNAL_STORAGE, "我们需要您允许我们读取你的存储卡", READ_EXTERNAL_STORAGE_CODE),
            new PermissionModel(
                    "存储空间", Manifest.permission.WRITE_EXTERNAL_STORAGE, "我们需要您允许我们读写你的存储卡，以方便我们临时保存一些数据", WRITE_EXTERNAL_STORAGE_CODE)
    };
    ...
    public void applyPermissions() {
        try {
            for (final PermissionModel model : mPermissionModels) {
                if (PackageManager.PERMISSION_GRANTED != checkSelfPermission(mActivity, model.permission)) {
        ...
        switch (requestCode) {
            ...
            case WRITE_EXTERNAL_STORAGE_CODE:
                ...
                if (PackageManager.PERMISSION_GRANTED != grantResults[0]) {
                    ...
                    // 方便用户理解我们为什么需要这个权限
                    if (shouldShowRequestPermissionRationale(mActivity, permissions[0])) {
